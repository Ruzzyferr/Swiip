...
const OGUN_KODLARI = ['kah', 'ogl', 'aks'] as const;
...
export default function OgunDestesi() {
  ...
  const yukle = useCallback(async () => {
    ...
    setIndeks(0);
    try {
      setDeste(await istek<DesteCevabi>(`/v1/ogun/deste?ogun=${ogun}&dolaptan=${dolaptan}`));
      setHata(null);
    } catch (h) {
  ...
  return (
    <>
      ...
      <Ekran>
        <Satir arasi="sm">
          {OGUN_KODLARI.map((kod) => (
            <Pressable
              key={kod}
              onPress={() => setOgun(kod)}
              accessibilityRole="tab"
              accessibilityState={{ selected: ogun === kod }}
              style={{
                flex: 1,
                minHeight: tema.dokunmaHedefi,
                borderRadius: tema.yaricap.md,
                borderWidth: ogun === kod ? 2 : 1,
                borderColor: ogun === kod ? tema.renk.aksan : tema.renk.cizgi,
                backgroundColor: ogun === kod ? tema.renk.aksanZemin : tema.renk.yuzey,
                alignItems: 'center',
                justifyContent: 'center',
              }}
            >
              <Yazi tur="kucuk" renk={ogun === kod ? 'aksan' : 'metinYumusak'}>
                {ogunAdlari[kod]}
              </Yazi>
            </Pressable>
          ))}
